...
int main(int argc, char **argv) {
    ...
    struct timespec a, b;
    unsigned int total;
    FILE *file = fopen("insert_wc_time.txt", "w");
    if (file == NULL) {
    ...
    for(int dataSize=tamMin; dataSize<=tamMax;dataSize+=increment){
        ...
        for (int i = 0; i < inter; i++) {
            // Inicializa o vetor com valores ordenados descrescentemente do tamanho do vetor a 0
    ...
    fclose(file);
    printf("Dados salvos no arquivo 'insert_wc_time.txt'.\n");
    return 0;
}
void insert_sort(int v[], int n){
    for(int i=1; i<n; i++){
